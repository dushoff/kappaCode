...
peakAssigner<-function(R0, y0 = 1e-9, finTime = 50){
  SIRsim<-sim(B0=R0, finTime=finTime, timeStep=0.0001,  y0 = y0)
  idx<-which.max(SIRsim$inc)
  #idx<-which.min(abs(SIRsim$x - 1/R0)) this one isn't accurate enough
...
cCalc_trunc <- function(time, cohort, sfun, cars=1, stopTime, B0){
	...
	return(list(
		cohort=cohort,  Rc=Rctot, varRc=(RcSS-Rctot^2)
		, RcSS =RcSS
		, cumden=cumden
		))	
		}
...
cohortStats_trunc <- function(B0
	...
	return(map_dfr(cohorts, function(c) {cCalc_trunc(sdat$time,
                                          cohort=c, 
                                          sfun=sfun,
                                          cars=cars,
                                          stopTime = stopTime,
...
truncSim <- function(B0
 ...
 , nIgnoredCohort =2){
    ...
    timeStep <- tpeak/nCohortPerIpeak
    ...
    maxInfCohort <- tpeak*(frcIpeak - nIgnoredCohort/nCohortPerIpeak) #removing a couple of last cohorts to avoid ode from throwing error
    cStats <- cohortStats_trunc( B0 = B0
                           , sdat=sdat
                           , maxInfCohort=maxInfCohort
                           , stopTime = frcIpeak*tpeak
                              )
   ifun <- approxfun(sdat$time, B0*sdat$x*sdat$y, rule=2)
   sfun <- approxfun(sdat$time, sdat$x, rule=2)
   rcfun <- approxfun(cStats$cohort, cStats$Rc, rule=2)
   varrcfun <- approxfun(cStats$cohort, cStats$varRc, rule=2)
   wssfun <- approxfun(cStats$cohort, cStats$RcSS, rule = 2)
   moments <- as.data.frame(ode(
             y=c(finS=0, mu=0, SS=0, V=0, w = 0, checkV = 0, correction = 0, intS=0)
              , func=v1ODE
              , times=sdat[sdat$time <= frcIpeak*tpeak, "time"]
              , parms=list( B0 = B0, ifun=ifun, rcfun=rcfun, varrcfun=varrcfun, wssfun = wssfun, sfun = sfun, y0 = y0)))

    ExpInf_index <- y0*cStats$Rc[1]
     yT <- sdat[which.min(abs(sdat$time - frcIpeak*tpeak)), "y"]
    ...
    SS <- mom$SS/finS
    w <- mom$w/finS
    checkV <- mom$checkV/finS
    within <- mom$V/finS
    between <- SS - mu^2
    total <- within + between
    otherCheck = w - mu^2
    mu_corrected <- (mom$mu + ExpInf_index)/finS
    fingerCrossed <- w + 2*(ExpInf_index + term)/finS
    #mu <- (mom$mu + ExpInf_index)/mom$finS
    Finalsize <- mom$finS
    return(data.frame(
                 timeStep=timeStep
                , B0 = B0
